    ...
    <section className="glass-panel panel-highlight rounded-[32px] p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-3 font-display text-2xl text-white">One-tap modes for the whole home</h2>
        ...
        {QUICK_SCENES.map((scene) => {
          ...
          return (
            <motion.button
              ...
              className={`rounded-[28px] border border-white/10 bg-gradient-to-br ${scene.gradient} p-[1px] text-left shadow-neon`}
            >
              <div className="h-full rounded-[27px] bg-slate-950/70 p-5">
                <div className="inline-flex rounded-2xl border border-white/10 bg-white/[0.04] p-3 text-white">
                  <Icon className="h-5 w-5" />
                </div>
                <h3 className="mt-5 font-display text-2xl text-white">{scene.label}</h3>
